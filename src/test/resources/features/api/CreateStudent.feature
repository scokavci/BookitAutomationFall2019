...
Feature: Create student
  ...
  Scenario: 2. Create student as a teacher and verify status code 201
    ...
    When user sends POST request to "/api/students/student" with following information:
      | first-name | last-name | email                        | password | role                | campus-location | batch-number | team-name      |
      | Lesly      | SDET      | myfriendsb15online@email.com | 1111     | student-team-member | VA              | 15           | Online_Hackers |
    And user verifies that response status code is 201

#  we can add only one student
#  so to resolve this issue, we can delete added student at the end of the case
  # in cucumber one column we use list
  # if we have more than one column we can use list of map
  # map structure requires two column and key value structure should be
  # dataTable is one particular data set
  #scenerio outline same test but for different set of data DDT
